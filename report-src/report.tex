...
\author{
	Fannar Freyr Jónuson \and Leonard Hastrup \and Petros Tsitsekidis \and Szymon Zalewski
}
\date{\today}
...
% (Target Length: 1 page)
% ---------------------------------
The user starts at the upset plot then sees that there is 60 referees, who have data from all three different leagues. This is something they find quite interesting, so the click on it and thus filters that data for the rest of the dashboard. They then go down to the choropleth map, and finds it interesting there are large countries like Russia that is not in this subset, prompting them to analyze Russian referees further. They thus click on Russia, and back in at the upset plot concludes that they represented in the champions league. Next observation is that Russia's strictness is comparable to other countries, so maybe this is a general thing for Eastern Europe. They thus switch to the region view, and wants to compare eastern and western europe to eachother. At first glance western europe seems more lenient, but looking at the scatterplot, this is likely caused by the low number of apperances for the very strict referees. They thus alter the minimum appearances and sees that they are in fact quite similiar to eachother. Then going further with the minimum appearnces they can thus see that there generally is a higher amount of reappeances for western referees. Which they then can continue to drill down on, also altering the strictness level to get a better idea of the tendency of the regions, also altering the strictness level to get a better idea of the tendency of the regions.
\begin{figure}[ht]
    \centering
    % \includegraphics[width=0.8\textwidth]{path/to/screenshot.png}
    \caption{Visualization showing the strictness discrepancy for English referees.}
    \label{fig:usecase}
\end{figure}
